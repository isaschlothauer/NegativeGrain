...
export async function checkDuplicates (username: string, email : string) {
  ...
  try {
    const [ username_login , _0 ] = await database.promise().query<UsernameDB[]>("SELECT id FROM users WHERE username = ?", [username])

    const [ email_login , _1 ] = await database.promise().query<emailDB[]>("SELECT id FROM users WHERE email = ?", [email])
